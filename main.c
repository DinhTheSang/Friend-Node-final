...
static void pri_level_request(uint16_t model_id, uint16_t element_index,
	...
	uint16 received_message = xor_decrypt(request->level);
	if(client_addr == 1){
		gateway_time_out = 0;
		return;
	}
	if (get_alarm_signal(received_message)){
		// chuyen? len gateway ngay;
		send_mesh_data(FLAG_NON_RESPONSE, FLAG_NON_RETRANS, received_message);
		return;
	}
	// thuc. hien cap nhat.
	int i = 0;
	for (; i < mesh_lpn_data_array.num_lpn; i++) {
		if(mesh_lpn_data_array.mesh_lpn_data[i].unicast_address == get_unicast_address(received_message))
		{
			mesh_lpn_data_array.mesh_lpn_data[i] = message2data(received_message);
			return;
		}
	}
